...
void Con_DrawSolidConsole( float frac ) {
	...
	static float conColorValue[4] = { 0.110f, 0.425f, 0.76f, 0.55f };
	...
	// draw the version number
	static float denzvercol[4] = { 0.8f, 0.8f, 0.25f, 1.0f };
	re.SetColor(denzvercol);
	SCR_DrawSmallString(cls.glconfig.vidWidth - (ARRAY_LEN(Q3_DENZVER)) * smallchar_width,
		lines - smallchar_height, Q3_DENZVER, ARRAY_LEN(Q3_DENZVER) - 1);
	...
	currentColorIndex = ColorIndex( COLOR_WHITE );
	static float conCol[] = {0.8f, 0.8f, 1.0f, 1.0f};
	re.SetColor( conCol );
